...
const ShopWithSidebar = ({ storeId }: { storeId?: string }) => {
  ...
  const [expansionsData, setExpansionsData] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([fetchExpansions, fetchAttributes])
      .then(([expansionsResponse, attributesResponse]) => {
        if (Array.isArray(expansionsResponse)) setExpansionsData(expansionsResponse);
  ...
  const handleExpansionSelect = (exp: string) => {
    if (exp === selectedExpansion) {
      setSelectedExpansion(null);
    } else {
      setSelectedExpansion(exp);
    }
    setSelectedAttribute(null);
    setCurrentPage(1);
  };
            ...
            <div className="xl:max-w-[870px] w-full">
              ...
              >
                {(() => {
                  if (isFetching) {
                    return (
                      <div className="flex justify-center py-20 w-full col-span-3">
                    ...
                    <ul className="flex items-center">
                      <li>
                        <button
                          ...
                          className="flex items-center justify-center w-8 h-9 ease-out duration-200 hover:text-blue disabled:text-gray-4"
                        >
                          <svg
                            className="fill-current"
                            width="18"
