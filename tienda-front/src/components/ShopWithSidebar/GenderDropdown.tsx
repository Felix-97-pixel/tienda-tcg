...
const EditionItem = ({ category, isSelected, onClick }) => {
  return (
    <button
      className={`${isSelected && "text-blue"
        } group flex items-start justify-between ease-out duration-200 hover:text-blue w-full text-left gap-2`}
      onClick={onClick}
    >
      <div className="flex items-start gap-2 flex-1">
        ...
        <span className="text-sm leading-tight break-words pr-2">{category.name}</span>
      </div>
...
const GenderDropdown = ({ expansions = [], selectedExpansion, onSelect }: any) => {
  ...
  return (
    <div className="bg-white shadow-1 rounded-lg">
      ...
      >
        {expansions.map((expansion: any, key: number) => (
          <EditionItem
            key={key}
            category={expansion}
            isSelected={selectedExpansion === expansion.name}
            onClick={() => onSelect(expansion.name)}
          />
        ))}
      </div>
    </div>
