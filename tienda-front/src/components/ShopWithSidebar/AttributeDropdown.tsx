...
const AttributeItem = ({ category, isSelected, onClick }) => {
  return (
    <button
...
const AttributeDropdown = ({ attributes = [], selectedAttribute, onSelect, title = "Atributos" }: any) => {
      ...
      >
        {attributes.map((attribute: any, key: number) => (
          <AttributeItem
            key={key}
            category={attribute}
            isSelected={selectedAttribute === attribute.name}
